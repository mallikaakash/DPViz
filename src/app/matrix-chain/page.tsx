...
const MatrixChain = () => {
  const [matrices, setMatrices] = useState<number[]>([40, 20, 30, 10, 30]);
  ...
  const matrixChainOrder = (p: number[]) => {
    ...
    const steps: number[][][] = [JSON.parse(JSON.stringify(m))];
    ...
    for (let L = 2; L <= n; L++) {
      for (let i = 1; i <= n - L + 1; i++) {
        ...
        m[i-1][j-1] = Infinity;
        for (let k = i; k <= j - 1; k++) {
          const q = m[i-1][k-1] + m[k][j-1] + p[i-1] * p[k] * p[j];
          if (q < m[i-1][j-1]) {
            m[i-1][j-1] = q;
          }
        }
        steps.push(JSON.parse(JSON.stringify(m)));
        explanations.push(`Calculating minimum multiplications for matrices ${i} to ${j}.`);
      }
    }
  ...
  useEffect(() => {
    const { steps, explanations } = matrixChainOrder(matrices);
    setMemo(steps[0]);
    setStep(0);
    ...
    >
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-8">
        ...
        <AnimatePresence>
          ...
          >
            ...
            <div className="space-y-6">
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.95 }}>
                <label className="block mb-2 text-lg font-medium text-gray-700">Matrix Dimensions:</label>
                <input
                  type="text"
                  value={matrices.join(', ')}
                  onChange={(e) => setMatrices(e.target.value.split(',').map(Number))}
              ...
              <thead>
                <tr>
                  ...
                  {memo[0] && memo[0].map((_, index) => (
                    <th key={index} className="px-4 py-2 border-b-2 border-gray-300 text-indigo-600">{index + 1}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {memo.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    <td className="px-4 py-2 border-b border-gray-300 font-semibold text-purple-600">{rowIndex + 1}</td>
          ...
          >
            <p className="text-lg"><strong className="text-indigo-600">Step:</strong> <span className="text-gray-700">{step} of {(matrices.length * (matrices.length - 1)) / 2}</span></p>
